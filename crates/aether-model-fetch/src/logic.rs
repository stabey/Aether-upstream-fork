...
pub fn build_models_fetch_url_for_client_version(
    ...
    let xai_media =
        provider_type == "xai" && matches!(api_format.as_str(), "openai:image" | "openai:video");
    if !endpoint_supports_rust_models_fetch(&api_format) && !xai_media {
        return None;
    }
...
pub fn selected_models_fetch_endpoints_for_provider(
    provider_type: &str,
    endpoints: &[StoredProviderCatalogEndpoint],
    key: &StoredProviderCatalogKey,
) -> Vec<StoredProviderCatalogEndpoint> {
    if !provider_type.trim().eq_ignore_ascii_case("xai") {
        return selected_models_fetch_endpoints(endpoints, key);
    }

    // xAI has one account catalog, including image/video models. A media-only key
    // can discover it too; a compact endpoint must not force OAuth onto the API host.
    ...
    .into_iter()
    .filter(|format| key_formats.is_empty() || key_formats.contains(*format))
    .find_map(|format| {
        endpoints.iter().find(|endpoint| {
            endpoint.is_active && normalize_api_format(&endpoint.api_format) == format
        })
    })
    .cloned()
    .into_iter()
...
pub(crate) fn normalize_cached_model(item: &Value, model_id: &str, api_format: &str) -> Value {
...
mod tests {
    ...
    #[test]
    fn xai_model_discovery_selects_one_active_endpoint_in_the_key_formats() {
        let formats = [
            "openai:responses",
            "openai:chat",
            "openai:responses:compact",
        ...
        let endpoints = formats
            .iter()
            .map(|format| sample_endpoint("provider-1", format, format, "https://api.x.ai/v1"))
            .collect::<Vec<_>>();
        for format in formats {
            let key = sample_key("provider-1", "key-1", &[format]);
            let selected =
                super::selected_models_fetch_endpoints_for_provider("xai", &endpoints, &key);
            assert_eq!(selected.len(), 1);
            assert_eq!(selected[0].api_format, format);
            let mut inactive = selected.clone();
            inactive[0].is_active = false;
            assert!(
                super::selected_models_fetch_endpoints_for_provider("xai", &inactive, &key)
                    .is_empty()
            );
        }
        let key = sample_key("provider-1", "key-1", &[]);
        let selected = super::selected_models_fetch_endpoints_for_provider("xai", &endpoints, &key);
        assert_eq!(selected.len(), 1);
        assert_eq!(selected[0].api_format, "openai:responses");
        assert_eq!(
            super::selected_models_fetch_endpoints_for_provider("custom", &endpoints, &key),
            selected_models_fetch_endpoints(&endpoints, &key)
        );
    }
    ...
    #[test]
    fn xai_discovery_never_substitutes_a_static_catalog() {
        assert!(preset_models_for_provider("xai").is_none());
        assert!(!super::provider_type_uses_preset_models("xai"));
    }
}
